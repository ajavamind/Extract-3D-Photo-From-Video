// Modified Video Export Library by Andy Modla
...
public class VideoExport {
  ...
  public VideoExport(PApplet parent, final String outputFileName,
    ...
    try {
      ...
      //        + "settings.json";
      settingsPath = parent.sketchPath("data") + File.separator
        + "settings.json";
      ...
      e.printStackTrace();
      err("Error loading settings.json");
    }
  ...
  public void setQuality(int crf, int audioBitRate) {
    if (ffmpeg != null) {
      err("Can't setQuality() after saveFrame()!");
      return;
    }
    if (crf > 100) {
      crf = 100;
  ...
  public void setFrameRate(float frameRate) {
    if (ffmpeg != null) {
      err("Can't setFrameRate() after saveFrame()!");
      return;
    }
  ...
  protected void initialize() {
    ...
    if (ffmpeg_path.equals(FFMPEG_PATH_UNSET)) {
      //JOptionPane.showMessageDialog(parent.frame,  // replaced by Andy Modla
      println(
  ...
  public void onFfmpegSelected(File selection) {
    if (selection == null) {
      ...
      println("ffmpeg selected at " + ffmpeg_path);
      settings.setString(SETTINGS_FFMPEG_PATH, ffmpeg_path);
      parent.saveJSONObject(settings, settingsPath);
      startFfmpeg(ffmpeg_path);
    }
  ...
  protected void err(String msg, int exitNo) {
    System.err.println("\nVideoExport error: " + msg + "\n");
    println("\nVideoExport error: " + msg + "\n");
    System.exit(exitNo);
  }
  ...
  protected void err(File f) {
    err("Ffmpeg failed. Study " + f + " for more details.", 2);
  }
